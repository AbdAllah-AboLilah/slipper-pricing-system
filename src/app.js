...
function resultStripHTML(l){return `<div class="pline-result four"><div class="pline-erp">اسم ERP<b data-mobile-erp>${erpNameCellHTML(l)}</b></div><div class="pline-erp">تكلفة القطعة<b data-mobile-cost>${money(l.costPerPiece)}</b></div><div class="pline-price">سعر البيع<b data-mobile-price>${l.finalPrice==null?'—':money(l.finalPrice)}</b></div><div class="pline-total">الإجمالي<b data-mobile-total>${money(l.total)}</b></div></div>`}
function detailsHTML(l){return `<details class="pline-details"><summary>تفاصيل الحساب</summary><div class="info-list tiny"><div>سعر الشراء/قطعة: <b>${money(l.purchasePerPiece)}</b></div><div>التكلفة/قطعة: <b>${money(l.costPerPiece)}</b></div><div>أساس التسعير: <b>${money(l.basisPrice)}</b></div><div>السعر قبل التقريب: <b>${l.prePrice==null?'—':money(l.prePrice)}</b></div><div>الباركود: <b>${esc(l.barcode||'—')}</b></div><div>الاي دي: <b>${esc(l.itemId||'—')}</b></div><div class="field"><label>ملاحظات</label><input data-k="notes" value="${esc(l.notes||'')}" placeholder="ملاحظة على الصنف ده"></div></div></details>`}
...
function lineRowCompact(l,i){
 ...
 return `<article class="itemrow itemrow-${cls}" data-line="${esc(l.id)}"><div class="itemrow-summary"><span class="itemrow-num">#${i+1}</span><span class="itemrow-name">${esc(title)}</span><span class="itemrow-mini">${num(l.pieces)} قطعة</span><span class="itemrow-mini">تكلفة <b data-mobile-cost>${money(l.costPerPiece)}</b></span><span class="itemrow-mini">${l.finalPrice==null?'—':money(l.finalPrice)}</span><span class="itemrow-mini strong">${money(l.total)}</span><span class="status ${cls}" data-mobile-status>${statusHTML(l)}</span><button class="btn danger small" data-del>حذف</button></div><details><summary>تعديل الصنف</summary>${fieldsHTML(l)}${l.status==='multiple-match'?resultStripHTML(l):''}${detailsHTML(l)}</details></article>`
}
...
async function renderInvoice(){
 ...
 if(!composeLine)composeLine=blankComposeLine(state.settings.defaultType||knownCategories()[0]||'');
 ...
 document.getElementById('imageSave').onclick=()=>saveImageOnly({...d,supplierName:supplierName(d.supplierId),invoiceDate:d.saved?d.invoiceDate:todayISO()});
...
function bindInvoice(d){
 ...
 document.getElementById('commitLine').onclick=async()=>{const calc=calcLine(composeLine,state.settings,state.tiers,state.items);if(!['ok','manual'].includes(calc.status)){const msg={'no-tier':'مفيش شريحة سعر تغطي السعر ده.','no-item':'مفيش صنف ERP بنفس السعر.','multiple-match':'محتاج تختار الصنف الصحيح الأول (فيه أكتر من صنف بنفس السعر).','manual-no-item':'السعر اليدوي مش مطابق لأي صنف ERP.'}[calc.status]||'الصنف محتاج مراجعة.';if(!confirm(`${msg}\nتضيفه للفاتورة برضه؟`))return}d.lines.push({...composeLine,id:uid('line')});d.type=calc.type||d.type;if(calc.type&&calc.type!==state.settings.defaultType){await setSetting('defaultType',calc.type);state.settings.defaultType=calc.type}composeLine=blankComposeLine(calc.type);scheduleAuto();render()};
...
function updateLineUI(card,l,idx,isCompose){
 const cls=lineStatusClass(l);
 if(isCompose){card.className=`pline pline-${cls}`}else{card.className=`itemrow itemrow-${cls}`;const nameEl=card.querySelector('.itemrow-name');if(nameEl)nameEl.textContent=l.status==='multiple-match'?'يحتاج اختيار الصنف':(l.erpName||'صنف بدون مطابقة');const mini=card.querySelectorAll('.itemrow-mini');if(mini[0])mini[0].textContent=`${num(l.pieces)} قطعة`;if(mini[2])mini[2].textContent=l.finalPrice==null?'—':money(l.finalPrice);if(mini[3])mini[3].textContent=money(l.total)}
 ...
 const costEl=card.querySelector('[data-mobile-cost]');if(costEl)costEl.textContent=money(l.costPerPiece);
...
async function renderCatalog(){
 ...
 <div class="card"><div class="toolbar space"><h3>بحث في الأصناف</h3><span class="pill">${state.items.length} صنف</span></div><input id="itemSearch" class="search" placeholder="اسم الصنف / الاي دي / باركود / مورد / قسم" value="${esc(state.filters.items)}"><div class="toolbar" style="margin-top:10px"><button class="btn danger" id="delSelectedItems">🗑️ حذف المحدد</button><button class="btn danger" id="delAllItems">🗑️ حذف كل الأصناف</button></div><div class="line-cards" style="margin-top:10px">${items.map(itemCardHTML).join('')||'<div class="empty">لا توجد بيانات.</div>'}</div></div>
 ...
 document.getElementById('delSelectedItems').onclick=async()=>{const ids=[...document.querySelectorAll('[data-isel]:checked')].map(c=>c.dataset.isel);if(!ids.length){toast('حدد أصناف الأول','error');return}if(!confirm(`حذف ${ids.length} صنف محدد؟`))return;for(const id of ids)await removeItem(id);state.items=await all('items');toast('تم الحذف','ok');render()};
 document.getElementById('delAllItems').onclick=async()=>{if(!state.items.length){toast('لا توجد أصناف','error');return}if(!confirm(`حذف كل الأصناف (${state.items.length})؟ ده مش قابل للتراجع.`))return;for(const it of state.items)await removeItem(it.id);state.items=await all('items');toast('تم حذف كل الأصناف','ok');render()};
 document.getElementById('delAllSuppliers')?.addEventListener('click',async()=>{if(!confirm(`حذف كل الموردين (${state.suppliers.length})؟ ده مش قابل للتراجع.`))return;for(const s of state.suppliers)await removeSupplier(s.id);state.suppliers=await all('suppliers');toast('تم حذف كل الموردين','ok');render()});
}
...
async function renderPricing(){
 ...
 document.getElementById('delSelectedTiers').onclick=async()=>{const ids=[...document.querySelectorAll('[data-tsel]:checked')].map(c=>c.dataset.tsel);if(!ids.length){toast('حدد شرائح الأول','error');return}if(!confirm(`حذف ${ids.length} شريحة محددة؟`))return;for(const id of ids)await removeTier(id);state.tiers=(await loadPricing()).tiers;toast('تم الحذف','ok');render()};
 document.getElementById('delAllTiers').onclick=async()=>{if(!state.tiers.length){toast('لا توجد شرائح','error');return}if(!confirm(`حذف كل الشرائح (${state.tiers.length})؟ ده مش قابل للتراجع.`))return;for(const t of state.tiers)await removeTier(t.id);state.tiers=(await loadPricing()).tiers;toast('تم حذف كل الشرائح','ok');render()};
