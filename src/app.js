...
import {APP_VERSION,CREATOR,BUILD_DATE,esc,money,num,todayISO,uid,toast,parseCSV,normalizeKey,ensureXLSX} from './core/utils.js';
...
async function renderInvoice(){
 ...
 bindInvoice(d);
 document.getElementById('addLine').onclick=()=>{d.lines.unshift({id:uid('line'),type:d.type,unit:12,qty:1,purchaseCartonPrice:0,discountRate:0,manualFinalPrice:'',notes:''});scheduleAuto();render()};
...
function itemCardHTML(x){
 return `<article class="pline pline-ok"><div class="pline-head"><b>${esc(x.name||'بدون اسم')}</b><span class="pill">${esc(x.type)}</span><button class="btn danger small" data-ri="${x.id}">حذف</button></div><div class="pline-result four"><div class="pline-erp"><span>ID</span><b>${esc(x.itemId||'—')}</b></div><div class="pline-erp"><span>الباركود</span><b>${esc(x.barcode||'—')}</b></div><div class="pline-erp"><span>المورد</span><b>${esc(x.supplier||'—')}</b></div><div class="pline-price"><span>سعر البيع بعد الخصم</span><b>${money(x.salePriceAfterDiscount)}</b></div></div><details class="pline-details"><summary>تفاصيل الصنف</summary><div class="info-list tiny"><div>القسم: <b>${esc(x.mainCategory||'—')} / ${esc(x.subCategory||'—')}</b></div><div>سعر البيع قبل الخصم: <b>${money(x.salePriceBeforeDiscount)}</b></div><div>نسبة الخصم: <b>${x.discountRate?num(x.discountRate)+'%':'—'}</b></div><div>قيمة الخصم: <b>${x.discountValue?money(x.discountValue):'—'}</b></div><div>سعر الشراء: <b>${money(x.purchasePrice)}</b></div><div>التكلفة: <b>${money(x.cost)}</b></div><div>LRP: <b>${esc(x.lrp||'—')}</b></div><div>LRB: <b>${esc(x.lrb||'—')}</b></div></div></details></article>`
}
async function renderCatalog(){
 ...
 <div class="card"><div class="toolbar space"><h3>بحث في الأصناف</h3><span class="pill">${state.items.length} صنف</span></div><input id="itemSearch" class="search" placeholder="اسم الصنف / ID / باركود / مورد / قسم / LRP / LRB" value="${esc(state.filters.items)}"><div class="line-cards" style="margin-top:10px">${items.map(itemCardHTML).join('')||'<div class="empty">لا توجد بيانات.</div>'}</div></div>
