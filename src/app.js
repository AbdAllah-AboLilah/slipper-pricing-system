...
function itemCardHTML(x){
 return `<article class="pline pline-ok"><div class="pline-head"><b>${esc(x.name||'بدون اسم')}</b><span class="pill">${esc(x.type)}</span><button class="btn danger small" data-ri="${x.id}">حذف</button></div><div class="pline-result four"><div class="pline-erp"><span>ID</span><b>${esc(x.itemId||'—')}</b></div><div class="pline-erp"><span>الباركود</span><b>${esc(x.barcode||'—')}</b></div><div class="pline-erp"><span>المورد</span><b>${esc(x.supplier||'—')}</b></div><div class="pline-price"><span>سعر البيع بعد الخصم</span><b>${money(x.salePriceAfterDiscount)}</b></div></div><details class="pline-details"><summary>تفاصيل الصنف</summary><div class="info-list tiny"><div>القسم: <b>${esc(x.mainCategory||'—')} / ${esc(x.subCategory||'—')}</b></div><div>سعر البيع قبل الخصم: <b>${money(x.salePriceBeforeDiscount)}</b></div><div>نسبة الخصم: <b>${x.discountRate?num(x.discountRate).toFixed(1)+'%':'—'}</b></div><div>قيمة الخصم: <b>${x.discountValue?money(x.discountValue):'—'}</b></div></div></details></article>`
}
...
function detectItemType(...texts){for(const t of texts){const n=normalizeKey(t||'');if(n.includes('winter'))return 'Winter';if(n.includes('سليبر')||n.includes('slipper'))return 'Slipper'}return ''}
...
function renderImportStep2(){const body=document.getElementById('importBody');body.innerHTML=`<div class="grid grid-2"><div class="field"><label>ورقة العمل</label><select id="sheetSelect">${importSession.workbook.sheets.map((s,i)=>`<option value="${i}">${esc(s.name)}</option>`).join('')}</select></div><div class="field"><label>النوع الافتراضي (لو لم يُكتشف النوع تلقائيًا من القسم الفرعي)</label><select id="importType">${itemTypeOptions(state.settings.defaultType||'Slipper')}</select></div></div><div class="note" style="margin-top:10px">النوع (Slipper/Winter) هيتحدد تلقائيًا لكل صف بمفرده من عمود "القسم الفرعي" (لو فيه كلمة winter أو سليبر). النوع الافتراضي فوق ده للصفوف اللي مش هيتم التعرف عليها تلقائيًا بس.</div><div id="sheetPreview" class="preview-grid" style="margin-top:12px"></div><button class="btn primary" id="toHeaders" style="margin-top:12px">التالي: اختيار صف العناوين</button>`;const update=()=>{const s=importSession.workbook.sheets[Number(document.getElementById('sheetSelect').value)];importSession.sheetIndex=Number(document.getElementById('sheetSelect').value);importSession.type=document.getElementById('importType').value;document.getElementById('sheetPreview').innerHTML=renderMatrixPreview(s.matrix,0,8,8)};document.getElementById('sheetSelect').onchange=update;document.getElementById('importType').onchange=e=>importSession.type=e.target.value;document.getElementById('toHeaders').onclick=()=>renderImportStep3();update()}
...
function renderImportStep5(){const s=importSession.workbook.sheets[importSession.sheetIndex],m=importSession.mapping,rows=s.matrix.slice(importSession.headerRow+1).filter(r=>r.some(v=>String(v??'').trim()));importSession.rows=rows;const sample=rows.slice(0,10);const val=(r,k)=>{const i=m[k];return i===''||i==null?'':r[Number(i)]??''};const typeCounts={Slipper:0,Winter:0,fallback:0};rows.forEach(r=>{const t=detectItemType(String(val(r,'subCategory')),String(val(r,'mainCategory')));if(t)typeCounts[t]++;else typeCounts.fallback++});document.getElementById('importBody').innerHTML=`<div class="summary"><div class="card"><div class="muted">عدد الصفوف</div><div class="stat">${rows.length}</div></div><div class="card"><div class="muted">Slipper (مكتشف)</div><div class="stat">${typeCounts.Slipper}</div></div><div class="card"><div class="muted">Winter (مكتشف)</div><div class="stat">${typeCounts.Winter}</div></div><div class="card"><div class="muted">افتراضي (${esc(importSession.type||'Slipper')})</div><div class="stat">${typeCounts.fallback}</div></div></div><div class="table-wrap" style="margin-top:12px"><table class="data-table"><thead><tr><th>اسم الصنف</th><th>ID</th><th>Barcode</th><th>المورد</th><th>القسم الفرعي</th><th>النوع المكتشف</th><th>قبل الخصم</th><th>بعد الخصم</th></tr></thead><tbody>${sample.map(r=>{const t=detectItemType(String(val(r,'subCategory')),String(val(r,'mainCategory')))||`${importSession.type||'Slipper'} (افتراضي)`;return `<tr><td>${esc(val(r,'name'))}</td><td>${esc(val(r,'itemId'))}</td><td>${esc(val(r,'barcode'))}</td><td>${esc(val(r,'supplier'))}</td><td>${esc(val(r,'subCategory'))}</td><td>${esc(t)}</td><td>${esc(val(r,'salePriceBeforeDiscount'))}</td><td>${esc(val(r,'salePriceAfterDiscount'))}</td></tr>`}).join('')}</tbody></table></div><div class="note" style="margin-top:12px">هيتم إنشاء الموردين تلقائيًا من عمود المورد، مع منع التكرار. نسبة الخصم وقيمته هيتحسبوا تلقائيًا من الفرق بين السعر قبل وبعد الخصم.</div><div class="toolbar" style="margin-top:12px"><button class="btn" id="backMap">رجوع</button><button class="btn primary" id="confirmImport">✅ تأكيد الاستيراد</button></div>`;document.getElementById('backMap').onclick=renderImportStep4;document.getElementById('confirmImport').onclick=commitImport}
async function commitImport(){const s=importSession.workbook.sheets[importSession.sheetIndex],m=importSession.mapping,rows=importSession.rows;const val=(r,k)=>{const i=m[k];return i===''||i==null?'':String(r[Number(i)]??'').trim()};let added=0,updated=0;const existing=await all('items');const byId=new Map(existing.filter(x=>x.itemId).map(x=>[String(x.itemId),x]));const suppliersByKey=new Map((await all('suppliers')).map(x=>[x.name,x]));const newItems=[];const newSuppliers=[];for(const r of rows){const name=val(r,'name');if(!name)continue;const itemId=val(r,'itemId'),supplier=val(r,'supplier'),subCategory=val(r,'subCategory'),mainCategory=val(r,'mainCategory');if(supplier&&!suppliersByKey.has(supplier)){const sp={id:uid('sup'),name:supplier,code:''};suppliersByKey.set(supplier,sp);newSuppliers.push(sp)}const before=num(val(r,'salePriceBeforeDiscount')),after=num(val(r,'salePriceAfterDiscount'));const discountValue=before>after?before-after:0;const discountRate=before>0?(discountValue/before*100):0;const type=detectItemType(subCategory,mainCategory)||importSession.type||'Slipper';const item={id:uid('item'),type,name,itemId,barcode:val(r,'barcode'),supplier,mainCategory,subCategory,salePriceBeforeDiscount:before,salePriceAfterDiscount:after,discountRate,discountValue,sellPrice:after};const old=itemId&&byId.get(itemId);if(old&&importSession.upsert==='update'){item.id=old.id;updated++}else added++;newItems.push(item)}if(newSuppliers.length)await putMany('suppliers',newSuppliers);if(newItems.length)await putMany('items',newItems);state.suppliers=await all('suppliers');state.items=await all('items');document.getElementById('importModal')?.remove();toast(`تم الاستيراد: ${added} جديد · ${updated} تحديث`,'ok');render()}
