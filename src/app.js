...
const IMPORT_FIELDS=[
 ['name','اسم الصنف'],['itemId','ID'],['barcode','الباركود'],['supplier','المورد'],['supplierCode','كود المورد'],['mainCategory','القسم الرئيسي'],['subCategory','القسم الفرعي'],['basePrice','السعر'],['salePriceBeforeDiscount','سعر البيع / قبل الخصم'],['salePriceAfterDiscount','السعر بعد الخصم'],['discountRate','نسبة الخصم %'],['discountValue','قيمة الخصم'],['purchasePrice','سعر الشراء'],['cost','التكلفة'],['lrp','LRP'],['lrb','LRB']
];
...
async function renderInvoice(){
 ...
 <div class="card"><div class="toolbar space"><div><h3>تفاصيل الفاتورة</h3><div class="muted tiny">سعر الشراء والتكلفة منفصلان. مثال: شراء 50 وخصم 5% = تكلفة 47.50.</div></div><button class="btn primary" id="addLine">+ إضافة صنف</button></div><div class="line-cards">${d.lines.length?d.lines.map(lineCardHTML).join(''):`<div class="empty">أضف أول صنف للبدء.</div>`}</div></div>
...
function bindInvoice(d){
 ...
 const bindLine=(card,id)=>{const l=d.lines.find(x=>x.id===id);const onchg=e=>{const el=e.target.closest('[data-k]');if(!el)return;const k=el.dataset.k;l[k]=el.value;Object.assign(l,calcLine(l,state.settings,state.tiers,state.items));updateLineUI(card,l,d.lines.indexOf(l));scheduleAuto()};card.addEventListener('input',onchg);card.addEventListener('change',onchg);card.querySelector('[data-del]')?.addEventListener('click',()=>{d.lines=d.lines.filter(x=>x.id!==id);scheduleAuto();render()})};
 document.querySelectorAll('.pline[data-line]').forEach(card=>bindLine(card,card.dataset.line));
}
function updateLineUI(card,l,idx){
 const cls=lineStatusClass(l);
 card.className=`pline pline-${cls}`;
 const title=l.status==='multiple-match'?'يحتاج اختيار الصنف':(l.erpName||'صنف جديد');
 card.querySelector('.pline-head b').textContent=`#${idx+1} · ${title}`;
 const statusEl=card.querySelector('[data-mobile-status]');statusEl.className=`status ${cls}`;statusEl.innerHTML=statusHTML(l);
 card.querySelector('[data-mobile-erp]').innerHTML=erpNameCellHTML(l);
 card.querySelector('[data-mobile-price]').textContent=l.finalPrice==null?'—':money(l.finalPrice);
 card.querySelector('[data-mobile-total]').textContent=money(l.total);
...
function guessMapping(headers){
 ...
 // من الأكثر تحديدًا إلى الأعم، عشان الأعمدة العامة (زي "السعر" أو "القسم") متاخدش عمود عمود تاني أدق منها.
 ...
 const discountValue=find(['قيمةالخصم','discountvalue','discountamount']);
 const discountRate=find(['نسبةالخصم','discountrate','discount%','الخصم','خصم']);
 const purchasePrice=find(['سعرالشراء','purchaseprice','purchase']);
 const cost=find(['التكلفة','cost']);
 const lrp=find(['lrp']);
 const lrb=find(['lrb']);
 const supplierCode=find(['كودالمورد','suppliercode']);
 ...
 const basePrice=find(['السعر','price']);
 return {name,itemId,barcode,supplier,supplierCode,mainCategory,subCategory,basePrice,salePriceBeforeDiscount,salePriceAfterDiscount,discountRate,discountValue,purchasePrice,cost,lrp,lrb};
}
...
async function commitImport(){const s=importSession.workbook.sheets[importSession.sheetIndex],m=importSession.mapping,rows=importSession.rows;const val=(r,k)=>{const i=m[k];return i===''||i==null?'':String(r[Number(i)]??'').trim()};let added=0,updated=0;const existing=await all('items');const byId=new Map(existing.filter(x=>x.itemId).map(x=>[String(x.itemId),x]));const suppliersByKey=new Map((await all('suppliers')).map(x=>[`${x.name}|${x.code||''}`,x]));const newItems=[];const newSuppliers=[];for(const r of rows){const name=val(r,'name');if(!name)continue;const itemId=val(r,'itemId'),supplier=val(r,'supplier'),supplierCode=val(r,'supplierCode');if(supplier){const key=`${supplier}|${supplierCode}`;if(!suppliersByKey.has(key)){const sp={id:uid('sup'),name:supplier,code:supplierCode};suppliersByKey.set(key,sp);newSuppliers.push(sp)}}const item={id:uid('item'),type:importSession.type||'Slipper',name,itemId,barcode:val(r,'barcode'),supplier,supplierCode,mainCategory:val(r,'mainCategory'),subCategory:val(r,'subCategory'),basePrice:num(val(r,'basePrice')),salePriceBeforeDiscount:num(val(r,'salePriceBeforeDiscount')),salePriceAfterDiscount:num(val(r,'salePriceAfterDiscount')),discountRate:num(val(r,'discountRate')),discountValue:num(val(r,'discountValue')),sellPrice:num(val(r,'salePriceAfterDiscount')),purchasePrice:num(val(r,'purchasePrice')),cost:num(val(r,'cost')),lrp:val(r,'lrp'),lrb:val(r,'lrb')};const old=itemId&&byId.get(itemId);if(old&&importSession.upsert==='update'){item.id=old.id;updated++}else added++;newItems.push(item)}if(newSuppliers.length)await putMany('suppliers',newSuppliers);if(newItems.length)await putMany('items',newItems);state.suppliers=await all('suppliers');state.items=await all('items');document.getElementById('importModal')?.remove();toast(`تم الاستيراد: ${added} جديد · ${updated} تحديث`,'ok');render()}
...
async function renderPricing(){
 ...
 document.getElementById('loadSlipperDefaults').onclick=()=>loadDefaults('Slipper',SLIPPER_DEFAULT_TIERS);
...
function erpKeys(){return [{key:'itemId',label:'ID'},{key:'barcode',label:'الباركود'},{key:'itemName',label:'اسم الصنف'},{key:'unitLabel',label:'الوحدة (قطعة - ثابتة)'},{key:'cost',label:'السعر (التكلفة/قطعة)'},{key:'purchasePrice',label:'سعر الشراء/قطعة'},{key:'qty',label:'الكمية'},{key:'sellPrice',label:'سعر البيع (بعد الخصم)'},{key:'saleBeforeDiscount',label:'سعر البيع / قبل الخصم'},{key:'seq',label:'مسلسل'},{key:'invoiceNo',label:'رقم الفاتورة'},{key:'date',label:'التاريخ'},{key:'supplier',label:'المورد'},{key:'supplierId',label:'كود المورد'},{key:'type',label:'النوع'},{key:'lrp',label:'LRP'},{key:'lrb',label:'LRB'},{key:'unit',label:'كمية القطع بالدستة/الكرتونة (داخلي)'},{key:'cartonQty',label:'كمية الدستة/الكرتونة'},{key:'purchaseCartonPrice',label:'سعر الشراء/الدستة أو الكرتونة'},{key:'total',label:'الإجمالي'},{key:'profit',label:'الربح'},{key:'margin',label:'نسبة الربح'},{key:'notes',label:'ملاحظات'}]}
